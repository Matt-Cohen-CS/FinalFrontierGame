...
public class LunarLanding3 implements Screen {
     Lunar game;
   ...
   public  SpriteBatch batch;
    ...
    int contactCount;
    boolean crashed;
    public LunarLanding3(Lunar gam) {
        this.game = gam;

        // Create sounds, this includes background music and both crashing and landing sounds
        backgroundMusic = Gdx.audio.newMusic(Gdx.files.internal("drumwarfare.mp3"));
        ...
        //Create background texture, sprite, and set positions and center alignment.
    ...
    public void render(float delta) {
        ...
        if (state == State.PAUSE){
            ...
            if(Gdx.input.isKeyPressed(Input.Keys.M)) {// sets screen to main menu
                game.setScreen(new MainMenuScreen(game));
                backgroundMusic.stop();
                dispose();
            }
        ...
        switch (state) {
            ...
            case RUN:
                ...
                if (TimeUtils.nanoTime() - lastDropTime > (1000000000 / 8)) {
                    spawnAsteroid();
                    spawnLaser();
                 }
                ...
                while (iter.hasNext()) {
                    ...
                    if(asteroid.overlaps(protRect) || asteroid.overlaps(protRect1) ){
                        iter.remove();
                    }
                }
